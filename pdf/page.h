...
class Page {
 public:
  ...
  // when displayed or printed. The value must always be in {0, 90, 180, 270}.
  // Any value not in that set is logged and ignored.
  void SetRotationDegrees(int degrees_clockwise);
  ...
  StatusOr<std::unique_ptr<ClientBitmap>> Render(float scale) const;

  // Given a bitmap, grab a region of the Page, and render it to the given
  // bitmap. The ClientBitmap provided to this function must have
  // BITMAP_FORMAT_RGBA_8888, and must be square. The requested region is
  // specified in a coordinate system where 0,0 is the lower-left corner of the
  // page, and width,height is the upper-right; that region must also be square.
  ...
  StatusOr<std::unique_ptr<PageObject>> GetPageObject(int i) const;

  // Takes ownership and deletes.
  ABSL_MUST_USE_RESULT Status
  RemovePageObject(std::unique_ptr<PageObject> object);

  StatusOr<std::unique_ptr<Annotation>> GetAnnotation(int index) const;
