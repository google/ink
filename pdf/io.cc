...
static constexpr float kAspectRatioWarningThreshold = 0.01;
...
static const glm::vec3 kCounterClockwise(0, 0, 1);
...
Status Write(const std::vector<proto::VectorElement>& vector_elements,
  ...
  for (const auto& element : vector_elements) {
    ...
    INK_RETURN_UNLESS(doc->CreatePath(
        geometry::Transform(glm::vec2(outline.x(0), outline.y(0)),
                            world_to_page),
        &path));
    for (int i = 1, n = outline.x_size(); i < n; i++) {
      path->LineTo(geometry::Transform(glm::vec2(outline.x(i), outline.y(i)),
                                       world_to_page));
    }
    path->Close();
    path->SetStrokeMode(StrokeMode::kNoStroke);
    path->SetFillMode(FillMode::kWinding);
...
Status Render(const ::ink::proto::ExportedDocument& exported_doc,
  ...
  for (int i = 0; i < pdf_document->PageCount(); ++i) {
    ...
    INK_RETURN_UNLESS(pdf_document->GetPage(i, &page));
    Rect world_bounds;
...
// was not found.
Status GetInkAnnotationV1(const Page& page, int* annot_index,
  ...
  *annot_index = -1;
  for (int i = 0; i < page.GetAnnotationCount(); i++) {
    auto status = page.GetAnnotation(i, annot);
    if (!status.ok()) {
      annot->reset();
      return status;
    }
...
Status ImportOnePath(const Path& path, const int page_num,
  ...
  INK_RETURN_UNLESS(path.GetCoordinates(&coords));
...
Status ReadAndStripV1(Page* page, const int page_num, glm::mat4 page_to_world,
  ...
  std::unique_ptr<Annotation> annot;
  int annot_index;
  auto status = GetInkAnnotationV1(*page, &annot_index, &annot);
  if (annot_index == -1) {
    // There is no Ink annotation to remove, nor to add to the export page.
    return ErrorStatus(StatusCode::NOT_FOUND, "no V1 ink annotation found");
  }
  if (!status.ok()) {
    return status;
  }
...
Status ReadAndStrip(Page* pdf_page, const int page_num,
  ...
  ink_page->set_background_has_transparency(pdf_page->HasTransparency());
  ...
  for (int i = 0; i < object_count; i++) {
    std::unique_ptr<PageObject> obj;
    INK_RETURN_UNLESS(pdf_page->GetPageObject(i, &obj));
    for (int mark_index = 0, mark_count = obj->MarkCount();
         mark_index < mark_count; mark_index++) {
      std::unique_ptr<PageObjectMark> mark;
      INK_RETURN_UNLESS(obj->GetMark(mark_index, &mark));
      std::string name;
      INK_RETURN_UNLESS(mark->GetName(&name));
...
Status ReadAndStrip(pdf::Document* pdf_doc,
  ...
  for (int i = 0; i < pdf_doc->PageCount(); i++) {
    std::unique_ptr<Page> page;
    auto status = pdf_doc->GetPage(i, &page);
    if (!status.ok()) {
      exported_doc->Clear();
      return status;
    }
    status = ReadAndStrip(page.get(), i, exported_doc);
    if (!status.ok()) {
      exported_doc->Clear();
      return status;
    }
