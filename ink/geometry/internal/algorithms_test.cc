...
TEST(LerpFloatTest, ExtremeBounds) {
  EXPECT_FLOAT_EQ(Lerp(-std::numeric_limits<float>::max(),
...
TEST(LerpAngleTest, ExtremeBounds) {
  EXPECT_THAT(Lerp(Angle::Radians(-std::numeric_limits<float>::max()),
                   Angle::Radians(std::numeric_limits<float>::max()), 0.5),
              AngleEq(Angle::Radians(0)));
}
