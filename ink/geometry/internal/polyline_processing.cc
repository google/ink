...
inline bool BetterLastIntersection(PolylineData& polyline,
  ...
  if (polyline.connect_first && !polyline.connect_last) {
    if (polyline.has_intersection) {
      // If an Intersection was previously found, we only update if the new
      // connection is closer to the end of the polyline.
      ...
      // If no Intersection was previously found, we just take the new
      // connection.
      return true;
    }
  }
...
inline bool BetterFirstIntersection(PolylineData& polyline,
                                    Intersection best_first_point_connection) {
  if (polyline.connect_last && !polyline.connect_first) {
    if (polyline.has_intersection) {
      // If an Intersection was previously found, we only update if the new
      // connection is closer to the start of the polyline.
      if ((best_first_point_connection.index_int <
