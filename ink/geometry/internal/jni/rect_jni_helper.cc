...
jobject CreateJImmutableBoxFromRectOrThrow(JNIEnv* env, Rect rect) {
  return env->CallStaticObjectMethod(
      ClassImmutableBox(), MethodImmutableBoxFromTwoPoints(),
...
void FillJMutableBoxFromRectOrThrow(JNIEnv* env, jobject mutable_box,
                                    Rect rect) {
  env->CallObjectMethod(mutable_box, MethodMutableBoxSetXBounds(), rect.XMin(),
                        rect.XMax());
  if (env->ExceptionCheck()) return;
  env->CallObjectMethod(mutable_box, MethodMutableBoxSetYBounds(), rect.YMin(),
                        rect.YMax());
}
