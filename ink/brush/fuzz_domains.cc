      ...
      [=](BrushPaint::TextureMapping mapping,
        ...
        return fuzztest::StructOf<BrushPaint>(
            VectorOf(ValidBrushPaintTextureLayerWithMappingAndAnimationFrames(
                mapping, std::get<0>(animation_frames_rows_columns),
                std::get<1>(animation_frames_rows_columns),
                std::get<2>(animation_frames_rows_columns), variant)));
      },
      ArbitraryBrushPaintTextureMapping(),
      Filter(
          [](std::tuple<int, int, int> animation_frames_rows_columns) {
            int frames = std::get<0>(animation_frames_rows_columns);
            int rows = std::get<1>(animation_frames_rows_columns);
            int columns = std::get<2>(animation_frames_rows_columns);
            // Equivalent to `frames <= rows * columns` but in a way that avoids
            // overflow.
            return frames / rows < columns ||
                   (frames / rows == columns && frames % rows == 0);
          },
          TupleOf(Positive<int>(), Positive<int>(), Positive<int>())));
}
