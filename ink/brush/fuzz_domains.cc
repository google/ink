...
fuzztest::Domain<absl::Duration> PositiveDuration() {
  return fuzztest::Filter(
      [](absl::Duration d) {
        return d != absl::ZeroDuration() && d != absl::InfiniteDuration();
      },
      fuzztest::Map(
          [](absl::Time a, absl::Time b) { return absl::AbsDuration(b - a); },
          fuzztest::Arbitrary<absl::Time>(),
          fuzztest::Arbitrary<absl::Time>()));
}
