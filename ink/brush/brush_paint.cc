...
absl::Status ValidateBrushPaintTextureLayer(
  ...
  if (layer.animation_rows <= 0) {
    return absl::InvalidArgumentError(
        absl::StrCat("`BrushPaint::TextureLayer::animation_rows` must be "
                     "strictly positive. Got ",
                     layer.animation_rows));
  }
  if (layer.animation_columns <= 0) {
    return absl::InvalidArgumentError(
        absl::StrCat("`BrushPaint::TextureLayer::animation_columns` must be "
                     "strictly positive. Got ",
  ...
  if (layer.animation_frames / layer.animation_rows > layer.animation_columns ||
      (layer.animation_frames / layer.animation_rows ==
           layer.animation_columns &&
       layer.animation_frames % layer.animation_rows != 0)) {
    return absl::InvalidArgumentError(absl::StrFormat(
