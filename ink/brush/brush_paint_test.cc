...
TEST(BrushPaintTest, BrushPaintSupportsAbslHash) {
  ...
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      ...
      BrushPaint{.self_overlap_visibility_preferences =
...
TEST(BrushPaintTest, BrushPaintEqualAndNotEqual) {
  ...
  other = paint;
  other.self_overlap_visibility_preferences.push_back(
      BrushPaint::SelfOverlapVisibility::kAccumulate);
  EXPECT_NE(paint, other);
}
...
TEST(BrushPaintTest, StringifyBrushPaint) {
  ...
  EXPECT_EQ(
      ...
      "blend_mode=kModulate}}, self_overlap_visibility_preferences={}}");
  EXPECT_EQ(
      absl::StrCat(
  ...
  EXPECT_EQ(
      ...
      "rotation_jitter=0π, opacity=1, animation_frames=1, animation_rows=1, "
      "animation_columns=1, animation_duration=1s, keyframes={}, "
      "blend_mode=kSrcIn}}, self_overlap_visibility_preferences={}}");
  EXPECT_EQ(
      absl::StrCat(BrushPaint{
          .texture_layers = {{.client_texture_id = std::string(kTestTextureId),
                              .mapping = BrushPaint::TextureMapping::kStamping,
  ...
  EXPECT_EQ(absl::StrCat(BrushPaint{
                .self_overlap_visibility_preferences =
                    {BrushPaint::SelfOverlapVisibility::kDiscardBasic}}),
            "BrushPaint{texture_layers={}, "
            "self_overlap_visibility_preferences={kDiscardBasic}}");
}
...
TEST(BrushPaintTest, DuplicateSelfOverlapVisibility) {
  EXPECT_THAT(
      brush_internal::ValidateBrushPaint(
          BrushPaint{.self_overlap_visibility_preferences =
                         {BrushPaint::SelfOverlapVisibility::kDiscardBasic,
