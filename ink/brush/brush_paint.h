...
struct BrushPaint {
  ...
  struct TextureLayer {
    ...
    // The length of time that it takes to loop through every one of the
    ...
    // Defaults to 1 second, but ignored if `animation_frames` is 1 (its default
    // value) because that indicates that animation is disabled. Must be a whole
    // number of milliseconds between 1 and 2^24 (inclusive).
