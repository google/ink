...
using OnDecodeMappedTexture = std::function<std::string(
    const std::string& encoded_id, VectorBitmap* absl_nullable bitmap)>;
...
absl::StatusOr<Brush> DecodeBrush(
    ...
    OnDecodeMappedTexture get_client_texture_id =
        [](const std::string& encoded_id, VectorBitmap* absl_nullable bitmap) {
          return encoded_id;
        });
