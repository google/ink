...
SmallArray<CodedNumericRun*, 4> InitCodedAttributeComponents(
  ...
  for (uint8_t c = 0; c < component_count; ++c) {
    CodedNumericRun* coded_component = coded_mesh.add_attribute_components();
    coded_component->mutable_deltas()->Reserve(vertex_count);
    coded_component->set_offset(coding_params.components[c].offset);
    coded_component->set_scale(coding_params.components[c].scale);
    coded_components[c] = coded_component;
  }
...
void EncodePackedMeshAttribute(const Mesh& mesh, uint32_t attribute_index,
  ...
  uint8_t component_count = MeshFormat::ComponentCount(
      mesh.Format().Attributes()[attribute_index].type);
  SmallArray<CodedNumericRun*, 4> coded_components =
      InitCodedAttributeComponents(
          mesh.VertexAttributeUnpackingParams(attribute_index), vertex_count,
          coded_mesh);
...
void EncodeUnpackedMeshAttribute(const Mesh& mesh, uint32_t attribute_index,
  ...
  SmallArray<CodedNumericRun*, 4> coded_components =
      InitCodedAttributeComponents(*coding_params, vertex_count, coded_mesh);
  SmallArray<int, 4> previous_integers(component_count, 0);
  for (uint32_t v = 0; v < vertex_count; ++v) {
...
void EncodeMeshOmittingFormat(const Mesh& mesh,
  ...
  coded_mesh.mutable_attribute_components()->Reserve(
      format.TotalComponentCount());
...
absl::StatusOr<Mesh> DecodeMeshUsingFormat(
  ...
  if (coded_mesh.attribute_components_size() == 0) {
    ...
    if (coded_mesh.attribute_components_size() != total_component_count) {
      return absl::InvalidArgumentError(absl::StrCat(
          "MeshFormat has ", total_component_count,
          " total components, but CodedMesh has ",
          coded_mesh.attribute_components_size(), " attribute_components"));
    }
    component_vectors.reserve(total_component_count);
    for (const CodedNumericRun& component : coded_mesh.attribute_components()) {
      absl::StatusOr<iterator_range<CodedNumericRunIterator<float>>> range =
          DecodeFloatNumericRun(component);
      if (!range.ok()) return range.status();
      component_vectors.push_back(
          std::vector<float>(range->begin(), range->end()));
    }
  }
