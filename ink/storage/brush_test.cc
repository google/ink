...
Version GetMaxProtoVersion(const ::google::protobuf::Message& message,
                           Version max_version) {
  ...
  if (descriptor->options().HasExtension(ink::proto::message_min_version)) {
    ...
    ABSL_CHECK_OK(message_version.status());
    max_version = MaxVersion(max_version, message_version.value());
  }
  ...
  for (const auto* field : fields) {
    if (field->options().HasExtension(ink::proto::field_min_version)) {
      ...
      ABSL_CHECK_OK(field_version.status());
      max_version = MaxVersion(max_version, field_version.value());
    }
    ...
    if (field->cpp_type() == ::google::protobuf::FieldDescriptor::CPPTYPE_ENUM) {
          ...
          if (enum_value->options().HasExtension(
            ...
            absl::StatusOr<Version> enum_value_version =
                DecodeBrushFamilyVersion(enum_value->options().GetExtension(
                    ink::proto::enum_value_min_version));
            ABSL_CHECK_OK(enum_value_version.status());
            max_version = MaxVersion(max_version, enum_value_version.value());
          }
        }
      } else {
        const ::google::protobuf::EnumValueDescriptor* enum_value =
      ...
      if (field->is_repeated()) {
        for (int i = 0; i < reflection->FieldSize(message, field); ++i) {
          max_version = GetMaxProtoVersion(
              reflection->GetRepeatedMessage(message, field, i), max_version);
        }
      } else {
        max_version = GetMaxProtoVersion(reflection->GetMessage(message, field),
                                         max_version);
      }
    }
  }
...
void CheckMinVersionExistsAndIsValid(
  ...
  absl::StatusOr<Version> message_version = DecodeBrushFamilyVersion(
      descriptor->options().GetExtension(ink::proto::message_min_version));
  ABSL_CHECK_OK(message_version.status());
  EXPECT_THAT(version::kMax, VersionGe(message_version.value()));
  ...
  for (int i = 0; i < descriptor->field_count(); ++i) {
    ...
    absl::StatusOr<Version> field_version = DecodeBrushFamilyVersion(
        field->options().GetExtension(ink::proto::field_min_version));
    ABSL_CHECK_OK(field_version.status());
    EXPECT_THAT(version::kMax, VersionGe(field_version.value()));
      ...
      for (int j = 0; j < enum_descriptor->value_count(); ++j) {
        ...
        absl::StatusOr<Version> enum_value_version =
            DecodeBrushFamilyVersion(enum_value->options().GetExtension(
                ink::proto::enum_value_min_version));
        ABSL_CHECK_OK(enum_value_version.status());
        EXPECT_THAT(version::kMax, VersionGe(enum_value_version.value()));
      }
    }
  }
  ...
  for (int i = 0; i < descriptor->nested_type_count(); ++i) {
    CheckMinVersionExistsAndIsValid(descriptor->nested_type(i),
...
void CalculateMinimumRequiredVersionMatchesProtoOptions(
  ...
  absl::StatusOr<Version> min_version =
      DecodeBrushFamilyVersion(family_proto.min_version());
  // Implicitly this also tests that version::kMax is greater than or equal to
  // the min version produced by CalculateMinimumRequiredVersion, otherwise
  // decoding would fail.
  ABSL_CHECK_OK(min_version);

  // Using reflection, examine the minimum required version of each
  // field/message/enum in the proto, and find the maximum.
  EXPECT_THAT(min_version.value(),
              VersionEq(GetMaxProtoVersion(
                  family_proto, Version(0, 0, 0, Version::Cycle::kStable, 0))));
}
...
TEST(BrushTest,
  ...
  // enums have a `minimum_version` option set, and it is valid (lower than or
  // equal to version::kMax). This ensures that they are well documented, valid
  // to be loaded, and will be covered by the
...
// Ensure that decoder rejects a proto with a min version higher than the
// current library version (which is also outside the range of known enum values
// for Version).
TEST(BrushTest, DecodeBrushFamilyFailsWithOutOfRangeMinVersion) {
  ...
  EXPECT_THAT(
      ...
      StatusIs(absl::StatusCode::kInvalidArgument,
               absl::StrCat("Failed to decode BrushFamily version: version ",
                            "2147483647.2147483647.2147483647",
                            " is greater than the maximum supported version ",
                            ToFormattedString(version::kMax))));
}

TEST(BrushTest, DecodeBrushFamilyFailsWithUnrecognizedCycle) {
  proto::BrushFamily family_proto;
  proto::Version* min_version = family_proto.mutable_min_version();
