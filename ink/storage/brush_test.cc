...
void EncodeDecodeMultipleBrushFamiliesRoundTrip(
  ...
  EXPECT_THAT(family_proto_in_filtered.newer_brush_families_size(),
              Eq(families_in_filtered.size() - 1));

  // Fuzz tests will generate brushes with development versions, so
  // max_version must be kDevelopment.
  absl::StatusOr<std::vector<BrushFamily>> families_out =
      DecodeMultipleBrushFamilies(family_proto_in, Version::kDevelopment());
  ASSERT_THAT(families_out.status(), IsOk());
  ...
  if (families_in_filtered.size() > 1) {
    ...
    ASSERT_THAT(out, IsOk());
    EXPECT_THAT(*out, BrushFamilyEq(families_in_filtered[0]));
    EXPECT_FALSE(out->HasFallbacks());
  }
}
...
TEST(BrushTest, DecodeBrushFamilyIsOkWithFallbacks) {
  proto::BrushFamily family_proto;
  family_proto.set_min_version(0);
  family_proto.add_newer_brush_families()->set_min_version(
      Version::k0Jetpack1_0_0().value());
  family_proto.add_newer_brush_families()->set_min_version(
      Version::k1Jetpack1_1_0Alpha01().value());
  family_proto.add_newer_brush_families()->set_min_version(
      Version::kDevelopment().value());
...
TEST(BrushTest, DecodeMultipleBrushFamiliesReturnsAllFallbacks) {
  proto::BrushFamily family_proto;
  family_proto.set_min_version(Version::k0Jetpack1_0_0().value());
  family_proto.add_newer_brush_families()->set_min_version(
      Version::k1Jetpack1_1_0Alpha01().value());
  family_proto.add_newer_brush_families()->set_min_version(
      Version::kDevelopment().value());
  ...
  ASSERT_THAT(families, IsOk());
  EXPECT_THAT(*families, SizeIs(3));
}
...
TEST(BrushTest, DecodeBrushFamilyReturnsMaxCompatibleVersion) {
  ...
  family_proto.add_newer_brush_families()->set_min_version(
      Version::k1Jetpack1_1_0Alpha01().value());
  proto::BrushFamily* newest_family_proto =
      family_proto.add_newer_brush_families();
  newest_family_proto->set_min_version(Version::kDevelopment().value());
  newest_family_proto->set_client_brush_family_id("newest_family");
  absl::StatusOr<BrushFamily> family =
...
TEST(BrushTest, DecodeBrushFamilyFailsWithNoCompatibleVersion) {
  proto::BrushFamily family_proto;
  family_proto.set_min_version(Version::k1Jetpack1_1_0Alpha01().value());
  family_proto.add_newer_brush_families()->set_min_version(
      Version::kDevelopment().value());
  EXPECT_THAT(
...
TEST(BrushTest, DecodeBrushFamilyPreservesOpaqueFallbacks) {
  proto::BrushFamily family_proto;
  family_proto.set_min_version(Version::k0Jetpack1_0_0().value());
  family_proto.set_client_brush_family_id("v0");

  proto::BrushFamily* newer_v1 = family_proto.add_newer_brush_families();
  newer_v1->set_min_version(Version::k1Jetpack1_1_0Alpha01().value());
  newer_v1->set_client_brush_family_id("v1");

  proto::BrushFamily* newer_v2 = family_proto.add_newer_brush_families();
  newer_v2->set_min_version(Version::kDevelopment().value());
  newer_v2->set_client_brush_family_id("v2");

  // Decode with k1Jetpack1_1_0Alpha01 (should pick "v1")
  absl::StatusOr<BrushFamily> family =
      DecodeBrushFamily(family_proto, Version::k1Jetpack1_1_0Alpha01());
  ASSERT_THAT(family, IsOk());
  EXPECT_EQ(family->GetMetadata().client_brush_family_id, "v1");
  EXPECT_TRUE(family->HasFallbacks());
...
TEST(BrushTest, DecodeBrushFamilyPreservesUnreadableFallbacks) {
  ...
  family_proto.set_min_version(Version::k0Jetpack1_0_0().value());
  family_proto.set_client_brush_family_id("v0");
  proto::BrushFamily* newer_family_proto =
      family_proto.add_newer_brush_families();
  const std::string text_proto = R"pb(
    client_brush_family_id: "v1000"
    min_version: 1000
    unrecognized_test_field: "Some unknown value"
  )pb";
  google::protobuf::TextFormat::Parser parser;
  parser.AllowUnknownField(true);
  ASSERT_TRUE(parser.ParseFromString(text_proto, newer_family_proto));
  // Decode with k0Jetpack1_0_0 (should pick "v0")
  absl::StatusOr<BrushFamily> family =
      DecodeBrushFamily(family_proto, Version::k0Jetpack1_0_0());
  ASSERT_THAT(family.status(), IsOk());
  EXPECT_THAT(family->GetMetadata().client_brush_family_id, Eq("v0"));
  EXPECT_TRUE(family->HasFallbacks());
  // Re-encode and verify result is identical to original proto
  proto::BrushFamily family_proto_out;
  EncodeBrushFamily(*family, family_proto_out);
  ...
  EXPECT_THAT(familyUnrecognized->GetMetadata().client_brush_family_id,
              Eq("v1000"));
  EXPECT_TRUE(familyUnrecognized->HasFallbacks());
...
TEST(BrushTest, HasFallbacksReturnsExpectedValue) {
  ...
  BrushFamilyInternalAccessor::SetOpaqueDecodedProtoBytesWithFallbacks(family,
                                                                       "bytes");
  EXPECT_TRUE(family.HasFallbacks());
  BrushFamilyInternalAccessor::SetOpaqueDecodedProtoBytesWithFallbacks(family,
                                                                       "");
...
TEST(BrushTest, EncodeMultipleBrushFamiliesIgnoresFallbacks) {
  ...
  opaque_family0.set_client_brush_family_id("opaque");
  ...
  BrushFamilyInternalAccessor::SetOpaqueDecodedProtoBytesWithFallbacks(
      *family0, opaque_family0.SerializeAsString());
  absl::StatusOr<BrushFamily> family1 = BrushFamily::Create(
      BrushTip({1, 1}, 1, Angle::Radians(0), 0, Angle::Radians(0), 0,
               Duration32::Zero(),
               {BrushBehavior(
  ...
  absl::StatusOr<std::vector<BrushFamily>> families =
      DecodeMultipleBrushFamilies(family_proto, Version::kDevelopment());
  ASSERT_THAT(families, IsOk());
  EXPECT_THAT(*families, SizeIs(2));
  // Should equal the true family values, not the ignored fallbacks.
  EXPECT_THAT((*families)[0], BrushFamilyEq(*family0));
  EXPECT_THAT((*families)[1], BrushFamilyEq(*family1));
}

TEST(BrushTest, EncodeMultipleBrushFamiliesEmptyDoesntChangeProto) {
  proto::BrushFamily family_proto;
  family_proto.set_min_version(Version::kDevelopment().value());
  family_proto.set_client_brush_family_id("development");
  EncodeMultipleBrushFamilies({}, family_proto);
  // Nothing to serialize, so no change to proto.
  EXPECT_THAT(family_proto.min_version(), Eq(Version::kDevelopment().value()));
  EXPECT_THAT(family_proto.client_brush_family_id(), Eq("development"));
}
...
TEST(BrushTest, EncodeDecodeMultipleBrushFamiliesRoundTrip) {
  ...
  EXPECT_THAT((*families)[0], BrushFamilyEq(family0));
  EXPECT_THAT((*families)[1], BrushFamilyEq(*family1));
  EXPECT_THAT((*families)[2], BrushFamilyEq(*family2));
}
...
TEST(BrushTest, EncodeBrushFamilyWithMalformedProtoBytesLosesFallbacksButIsOk) {
  ...
  absl::StatusOr<BrushFamily> family_out =
      DecodeBrushFamily(family_proto_out, Version::kDevelopment());
  ASSERT_THAT(family_out, IsOk());
  EXPECT_THAT(*family_out, BrushFamilyEq(*family));
  EXPECT_FALSE(family_out->HasFallbacks());
}
