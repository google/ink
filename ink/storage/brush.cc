...
absl::StatusOr<BrushFamily> DecodeBrushFamily(
  ...
  auto old_id_to_bitmap = family_proto.texture_id_to_bitmap();
  ...
  ClientTextureIdProvider texture_callback =
    ...
    std::string new_id;
    auto bitmap_it = family_proto.texture_id_to_bitmap().find(old_id);
    if (bitmap_it != family_proto.texture_id_to_bitmap().end()) {
      std::string bitmap = bitmap_it->second;
      new_id = get_client_texture_id(old_id, &bitmap);
    } else {
