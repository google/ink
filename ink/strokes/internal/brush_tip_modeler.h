...
class BrushTipModeler {
 public:
  ...
  // states and resetting all stateful behaviors. This is used when
  // previously-fixed tip states need to be re-modeled, due to e.g. a
  // `kTimeSinceStrokeEnd` behavior.
  void RestartStroke();
  ...
  /// be called just prior to calling `UpdateStroke()` with a newer input
