...
TEST(StrokeVertexTest, LerpNonPositionAttributes) {
  ...
  EXPECT_FLOAT_EQ(result.animation_offset, 0.95);
  ...
  EXPECT_FLOAT_EQ(result.animation_offset, 0);
  ...
  EXPECT_FLOAT_EQ(result.animation_offset, 0.125);
  ...
  EXPECT_FLOAT_EQ(result.animation_offset, 0.6);
}
...
TEST(StrokeVertexTest, BarycentricLerpNonPositionAttributes) {
  ...
  EXPECT_FLOAT_EQ(result.animation_offset, 0.375);
...
TEST(StrokeVertexTest, SetAnimationOffsetInMesh) {
  MutableMesh mesh(StrokeVertex::FullMeshFormat());
  mesh.Resize(5, 3);

  uint32_t index = 1;
  ASSERT_FLOAT_EQ(StrokeVertex::GetFromMesh(mesh, index)
                      .non_position_attributes.animation_offset,
                  0);

  StrokeVertex::SetAnimationOffsetInMesh(mesh, index, 0.75);
  EXPECT_FLOAT_EQ(StrokeVertex::GetFromMesh(mesh, index)
                      .non_position_attributes.animation_offset,
                  0.75);
}
