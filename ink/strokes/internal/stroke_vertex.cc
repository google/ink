...
std::optional<MeshAttributeCodingParams> GetCustomPackingParams(
  ...
  // Brush paint animation offsets are stored unpacked in the range [0, 1). It's
  // tempting to use 1/256 as the scale here, since a value of 1 does not need
  // to be representable, but due to rounding this would make values just less
  // than 1 also unrepresentable (see b/432526862), so we use 1/255 instead.
  // LINT.IfChange(anim_packing)
  constexpr MeshAttributeCodingParams::ComponentCodingParams
      kAnimationCodingParams8bit = {.scale = 1.f / 255.f};
  // LINT.ThenChange(
