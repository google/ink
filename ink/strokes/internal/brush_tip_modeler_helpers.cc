...
struct BrushTipStateModifiers {
  ...
  float paint_animation_progress_offset = 0;  // always in range [0, 1)
  ...
  switch (target) {
    ...
    case BrushBehavior::Target::kPaintAnimationProgressOffset:
      tip_state_modifiers.paint_animation_progress_offset = FloatModulo(
          tip_state_modifiers.paint_animation_progress_offset + modifier, 1);
      break;
...
void ApplyModifiersToTipState(const BrushTipStateModifiers& modifiers,
  ...
  if (modifiers.width_multiplier != 1) {
    tip_state.width = NanSafeMultiply(
        tip_state.width, std::max(modifiers.width_multiplier, 0.f));
  }
  if (modifiers.height_multiplier != 1) {
    tip_state.height = NanSafeMultiply(
        tip_state.height, std::max(modifiers.height_multiplier, 0.f));
  }
